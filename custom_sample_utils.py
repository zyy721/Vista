    ...
    for key in keys:
        if key in value_dict:
            if key in ["fps", "fps_id", "motion_bucket_id", "cond_aug"]:
                ...
                batch[key] = torch.tensor([value_dict[key]]).to(device)
...
def get_condition(model, value_dict, num_samples, force_uc_zero_embeddings, device):
    ...
    # c["crossattn"] = c["crossattn"].unsqueeze(0)
    # c["crossattn"] = repeat(c["crossattn"], "1 ... -> t ...", t=num_samples)
    # c["concat"] = c["concat"].unsqueeze(0)
    # c["concat"] = repeat(c["concat"], "1 ... -> t ...", t=num_samples)

    # uc["crossattn"] = uc["crossattn"].unsqueeze(0)
    # uc["crossattn"] = repeat(uc["crossattn"], "1 ... -> t ...", t=num_samples)
    # uc["concat"] = uc["concat"].unsqueeze(0)
    # uc["concat"] = repeat(uc["concat"], "1 ... -> t ...", t=num_samples)
...
def do_sample(
    ...
    with torch.no_grad(), precision_scope(device), model.ema_scope("Sampling"):
        ...
        z = rearrange(z, "(n_cam t) c h w -> t n_cam c h w", t=num_frames)
        ...
        samples_z = rearrange(samples_z, "t n_cam c h w -> (n_cam t) c h w", t=num_frames)
        ...
        samples = rearrange(samples, "(n_cam t) c h w -> n_cam t c h w", t=num_frames)
        samples_z = rearrange(samples_z, "(n_cam t) c h w -> n_cam t c h w", t=num_frames)
        images = rearrange(images, "(n_cam t) c h w -> n_cam t c h w", t=num_frames)
